...
function adapter(opts){
  ...
  if (!sub) sub = createClient({ dropBufferSupport: false });

  var subJson = createClient({ dropBufferSupport: true });
  ...
  RedisAdapter.prototype.onclients = function(channel, msg){
   ...
   * @param {Object} fn callback (optional)
   * @api public
   */

  RedisAdapter.prototype.clients = function(rooms, fn){
  ...
  RedisAdapter.uid = uid;
  RedisAdapter.pubClient = pub;
  RedisAdapter.subClient = sub;
  RedisAdapter.prefix = prefix;
  RedisAdapter.clientsTimeout = clientsTimeout;
